...
pub struct ParametricRoman {
    base: ParametricFunction,
    radius: f64,
}

impl ParametricRoman {
    /// VTK: `vtkParametricRoman::New`.
    pub fn new() -> Self {
        let mut base = ParametricFunction::with_class_name("vtkParametricRoman");
    ...
    /// VTK: `vtkParametricRoman::SetRadius`.
    ...
    /// VTK: `vtkParametricRoman::GetRadius`.
...
impl ParametricFunctionApi for ParametricRoman {
    ...
    /// VTK: `vtkParametricRoman::GetDimension`.
    fn get_dimension(&self) -> i32 {
        2
    }

    /// VTK: `vtkParametricRoman::Evaluate`.
    fn evaluate(&self, uvw: [f64; 3], pt: &mut [f64; 3], duvw: &mut [f64; 9]) {
        ...
        let s2v = (2.0 * v).sin();
        let sv = v.sin();
        let a2 = self.radius * self.radius;
        ...
        duvw[0] = a2 * cv2 * c2u;
        duvw[1] = a2 * cu * s2v / 2.0;
        duvw[2] = -a2 * su * s2v / 2.0;
    ...
    /// VTK: `vtkParametricRoman::EvaluateScalar`.
    fn evaluate_scalar(&self, _uvw: [f64; 3], _pt: [f64; 3], _duvw: [f64; 9]) -> f64 {
        0.0
    }
}

impl Default for ParametricRoman {
    fn default() -> Self {
        Self::new()
    }
}
