...
pub struct ParametricPluckerConoid {
    base: ParametricFunction,
    n: i32,
}

impl ParametricPluckerConoid {
    /// VTK: `vtkParametricPluckerConoid::New`.
    pub fn new() -> Self {
        let mut base = ParametricFunction::with_class_name("vtkParametricPluckerConoid");
        base.set_minimum_u(0.0);
        base.set_maximum_u(3.0);
    ...
    /// VTK: `vtkParametricPluckerConoid::SetN`.
    ...
    /// VTK: `vtkParametricPluckerConoid::GetN`.
...
impl ParametricFunctionApi for ParametricPluckerConoid {
    ...
    /// VTK: `vtkParametricPluckerConoid::GetDimension`.
    fn get_dimension(&self) -> i32 {
        2
    }

    /// VTK: `vtkParametricPluckerConoid::Evaluate`.
    fn evaluate(&self, uvw: [f64; 3], pt: &mut [f64; 3], duvw: &mut [f64; 9]) {
        let u = uvw[0];
        let v = uvw[1];
        let n_v = self.n as f64 * v;
        ...
        duvw[3] = u * v.cos();
        duvw[4] = -u * v.sin();
        duvw[5] = self.n as f64 * n_v.cos();
    }

    /// VTK: `vtkParametricPluckerConoid::EvaluateScalar`.
    fn evaluate_scalar(&self, _uvw: [f64; 3], _pt: [f64; 3], _duvw: [f64; 9]) -> f64 {
        0.0
    }
}

impl Default for ParametricPluckerConoid {
    fn default() -> Self {
        Self::new()
    }
}
