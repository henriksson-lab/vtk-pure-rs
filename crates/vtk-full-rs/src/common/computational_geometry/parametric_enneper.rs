...
pub struct ParametricEnneper {
    base: ParametricFunction,
}

impl ParametricEnneper {
    /// VTK: `vtkParametricEnneper::New`.
    pub fn new() -> Self {
        let mut base = ParametricFunction::with_class_name("vtkParametricEnneper");
        base.set_minimum_u(-2.0);
        base.set_maximum_u(2.0);
        base.set_minimum_v(-2.0);
...
impl ParametricFunctionApi for ParametricEnneper {
    ...
    /// VTK: `vtkParametricEnneper::GetDimension`.
    fn get_dimension(&self) -> i32 {
        2
    }

    /// VTK: `vtkParametricEnneper::Evaluate`.
    fn evaluate(&self, uvw: [f64; 3], pt: &mut [f64; 3], duvw: &mut [f64; 9]) {
        ...
        pt[0] = u - u * u * u / 3.0 + u * v * v;
        pt[1] = v - v * v * v / 3.0 + u * u * v;
        pt[2] = u * u - v * v;
    ...
    /// VTK: `vtkParametricEnneper::EvaluateScalar`.
    fn evaluate_scalar(&self, _uvw: [f64; 3], _pt: [f64; 3], _duvw: [f64; 9]) -> f64 {
        0.0
    }
}

impl Default for ParametricEnneper {
    fn default() -> Self {
        Self::new()
    }
}
