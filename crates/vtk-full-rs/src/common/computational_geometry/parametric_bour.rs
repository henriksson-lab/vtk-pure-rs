...
/// VTK: `vtkParametricBour`.
#[derive(Debug, Clone, PartialEq)]
pub struct ParametricBour {
    base: ParametricFunction,
}

impl ParametricBour {
    /// VTK: `vtkParametricBour::New`.
    pub fn new() -> Self {
        let mut base = ParametricFunction::with_class_name("vtkParametricBour");
        base.set_minimum_u(0.0);
        base.set_maximum_u(1.0);
        base.set_minimum_v(0.0);
        base.set_maximum_v(4.0 * pi());
...
impl ParametricFunctionApi for ParametricBour {
    ...
    /// VTK: `vtkParametricBour::GetDimension`.
    fn get_dimension(&self) -> i32 {
        2
    }

    /// VTK: `vtkParametricBour::Evaluate`.
    fn evaluate(&self, uvw: [f64; 3], pt: &mut [f64; 3], duvw: &mut [f64; 9]) {
        ...
        let u3_2 = u * u1_2;
        let cos_v = v.cos();
        let cos_1_5v = (1.5 * v).cos();
        let cos_2v = (2.0 * v).cos();
        let sin_v = v.sin();
        let sin_1_5v = (1.5 * v).sin();

        pt[0] = u * cos_v - u * u * cos_2v / 2.0;
        pt[1] = -u * sin_v * (u * cos_v + 1.0);
        pt[2] = 4.0 / 3.0 * u3_2 * cos_1_5v;

        duvw[0] = cos_v - u * cos_2v;
        duvw[1] = -sin_v * (1.0 + 2.0 * u * cos_v);
        duvw[2] = 2.0 * u1_2 * cos_1_5v;

        duvw[3] = u * (2.0 * u * cos_v - 1.0) * sin_v;
        duvw[4] = -u * (cos_v + u * cos_2v);
        duvw[5] = -2.0 * u3_2 * sin_1_5v;
    }

    /// VTK: `vtkParametricBour::EvaluateScalar`.
    fn evaluate_scalar(&self, _uvw: [f64; 3], _pt: [f64; 3], _duvw: [f64; 9]) -> f64 {
        0.0
    }
}

impl Default for ParametricBour {
    fn default() -> Self {
        Self::new()
    }
}
