...
pub struct ParametricCrossCap {
    base: ParametricFunction,
}

impl ParametricCrossCap {
    /// VTK: `vtkParametricCrossCap::New`.
    pub fn new() -> Self {
        let mut base = ParametricFunction::with_class_name("vtkParametricCrossCap");
        base.set_minimum_u(0.0);
        base.set_maximum_u(std::f64::consts::PI);
        base.set_minimum_v(0.0);
        base.set_maximum_v(std::f64::consts::PI);
...
impl ParametricFunctionApi for ParametricCrossCap {
    ...
    /// VTK: `vtkParametricCrossCap::GetDimension`.
    fn get_dimension(&self) -> i32 {
        2
    }

    /// VTK: `vtkParametricCrossCap::Evaluate`.
    fn evaluate(&self, uvw: [f64; 3], pt: &mut [f64; 3], duvw: &mut [f64; 9]) {
        ...
        duvw[0] = -pt[1];
        duvw[1] = pt[0];
        duvw[2] = 2.0 * cu * su * sv * sv;
        duvw[3] = 2.0 * cu * c2v;
        duvw[4] = 2.0 * su * c2v;
        duvw[5] = -2.0 * cv * sv * (1.0 + cu * cu);
    }

    /// VTK: `vtkParametricCrossCap::EvaluateScalar`.
    fn evaluate_scalar(&self, _uvw: [f64; 3], _pt: [f64; 3], _duvw: [f64; 9]) -> f64 {
        0.0
    }
}

impl Default for ParametricCrossCap {
    fn default() -> Self {
        Self::new()
    }
}
