...
pub struct ParametricHenneberg {
    base: ParametricFunction,
}

impl ParametricHenneberg {
    /// VTK: `vtkParametricHenneberg::New`.
    pub fn new() -> Self {
        let mut base = ParametricFunction::with_class_name("vtkParametricHenneberg");
        base.set_minimum_u(-1.0);
...
impl ParametricFunctionApi for ParametricHenneberg {
    ...
    /// VTK: `vtkParametricHenneberg::GetDimension`.
    fn get_dimension(&self) -> i32 {
        2
    }

    /// VTK: `vtkParametricHenneberg::Evaluate`.
    fn evaluate(&self, uvw: [f64; 3], pt: &mut [f64; 3], duvw: &mut [f64; 9]) {
        let u = uvw[0];
        let v = uvw[1];

        let sinh_u = u.sinh();
        let sinh_2u = (2.0 * u).sinh();
        let sinh_3u = (3.0 * u).sinh();
        let cosh_u = u.cosh();
        let cosh_2u = (2.0 * u).cosh();
        let cosh_3u = (3.0 * u).cosh();
        let sin_v = v.sin();
        let sin_2v = (2.0 * v).sin();
        let sin_3v = (3.0 * v).sin();
        let cos_v = v.cos();
        let cos_2v = (2.0 * v).cos();
        let cos_3v = (3.0 * v).cos();

        duvw.fill(0.0);

        pt[0] = 2.0 * sinh_u * cos_v - 2.0 / 3.0 * sinh_3u * cos_3v;
        pt[1] = 2.0 * sinh_u * sin_v + 2.0 / 3.0 * sinh_3u * sin_3v;
        pt[2] = 2.0 * cosh_2u * cos_2v;

        duvw[0] = 2.0 * cosh_u * cos_v - 2.0 * cosh_3u * cos_3v;
        duvw[1] = 2.0 * cosh_u * sin_v + 2.0 * cosh_3u * sin_3v;
        duvw[2] = 4.0 * sinh_2u * cos_2v;

        duvw[3] = -2.0 * sinh_u * sin_v + 2.0 * sinh_3u * sin_3v;
        duvw[4] = 2.0 * sinh_u * cos_v + 2.0 * sinh_3u * cos_3v;
        duvw[5] = -4.0 * cosh_2u * sin_2v;
    }

    /// VTK: `vtkParametricHenneberg::EvaluateScalar`.
    fn evaluate_scalar(&self, _uvw: [f64; 3], _pt: [f64; 3], _duvw: [f64; 9]) -> f64 {
        0.0
    }
}

impl Default for ParametricHenneberg {
    fn default() -> Self {
        Self::new()
    }
}
