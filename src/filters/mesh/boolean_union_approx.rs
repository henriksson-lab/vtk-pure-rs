...
fn collect_triangles(pd: &PolyData) -> Vec<Triangle> {
    ...
    for cell in pd.polys.iter() {
        ...
        let Some(p0) = point_for_cell_id(pd, cell[0]) else {
            continue;
        };
        for i in 1..cell.len() - 1 {
            let (Some(p1), Some(p2)) = (
                point_for_cell_id(pd, cell[i]),
                point_for_cell_id(pd, cell[i + 1]),
            ) else {
                continue;
            };
            tris.push((p0, p1, p2));
...
fn point_for_cell_id(pd: &PolyData, id: i64) -> Option<[f64; 3]> {
    let idx = usize::try_from(id).ok()?;
    (idx < pd.points.len()).then(|| pd.points.get(idx))
}
...
fn solid_angle(p: [f64; 3], tri: [[f64; 3]; 3]) -> f64 {
    ...
    if det.abs() < 1e-15 && denom.abs() < 1e-15 {
        return 0.0;
    }
    2.0 * det.atan2(denom)
}
