...
fn face_normal(cell: &[i64], mesh: &PolyData) -> [f64; 3] {
    ...
    if !cell.iter().all(|&id| valid_point_id(id, npts).is_some()) {
        return [0.0, 0.0, 1.0];
    }
    let mut n = [0.0, 0.0, 0.0];
    for i in 0..cell.len() {
        let current = mesh.points.get(cell[i] as usize);
        let next = mesh.points.get(cell[(i + 1) % cell.len()] as usize);
...
mod tests {
    ...
    #[test]
    fn feature_normal_uses_whole_polygon() {
        let mut mesh = PolyData::new();
        mesh.points.push([0.0, 0.0, 0.0]);
        mesh.points.push([0.5, 0.0, 0.0]);
        mesh.points.push([1.0, 0.0, 0.0]);
        mesh.points.push([1.0, 1.0, 0.0]);
        mesh.points.push([0.0, 1.0, 0.0]);
        mesh.points.push([0.5, 0.0, 1.0]);
        mesh.points.push([1.0, 0.0, 1.0]);
        mesh.points.push([2.0, 0.5, 0.0]);
        mesh.polys.push_cell(&[0, 1, 2, 3, 4]);
        mesh.polys.push_cell(&[1, 5, 6, 2]);
        mesh.polys.push_cell(&[2, 7, 3]);

        let result = smooth_preserve_boundary(&mesh, 1, 1.0, 45.0);

        assert_eq!(result.points.get(1), mesh.points.get(1));
        assert_eq!(result.points.get(2), mesh.points.get(2));
    }
}
