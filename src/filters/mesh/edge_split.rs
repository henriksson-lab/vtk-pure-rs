...
pub fn split_sharp_edges(input: &PolyData, angle_threshold_deg: f64) -> PolyData {
    ...
    let normals: Vec<[f64; 3]> = cells
        ...
        .map(|c| {
            ...
            let Some(i0) = valid_point_index(c[0], input.points.len()) else {
                return [0.0; 3];
            };
            let Some(i1) = valid_point_index(c[1], input.points.len()) else {
                return [0.0; 3];
            };
            let Some(i2) = valid_point_index(c[2], input.points.len()) else {
                return [0.0; 3];
            };
