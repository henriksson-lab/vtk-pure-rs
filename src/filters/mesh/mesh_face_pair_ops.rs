...
pub fn swap_diagonals_where_improves(mesh: &PolyData) -> PolyData {
    ...
    let mut swapped = vec![false; nc];
    for (&(ea, eb), faces) in &ef {
        if faces.len() != 2 || swapped[faces[0]] || swapped[faces[1]] {
            continue;
        }
        if cells[faces[0]].len() != 3 || cells[faces[1]].len() != 3 {
            continue;
        }
        ...
        if let (Some(va), Some(vb)) = (va, vb) {
            ...
            if new_min > old_min {
                ...
                new_cells[faces[1]] = new_t2;
                swapped[faces[0]] = true;
                swapped[faces[1]] = true;
            }
        }
    }
    ...
    for cell in [a, b] {
        ...
        for i in 0..3 {
            ...
            if l1 > 1e-15 && l2 > 1e-15 {
                mn = mn.min((d / (l1 * l2)).clamp(-1.0, 1.0).acos().to_degrees());
            }
        }
    }
    mn
...
mod tests {
    ...
    fn test() {
        ...
        let r = swap_diagonals_where_improves(&m);
