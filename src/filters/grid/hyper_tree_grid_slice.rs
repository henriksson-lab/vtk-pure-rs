...
pub fn hyper_tree_grid_slice(
    ...
    let cell_bounds = slice_cell_bounds(htg);
    ...
    for (cell_index, bounds) in cell_bounds.iter().enumerate() {
        // 8 corners of the cell (or 4 for 2D)
        let corners: Vec<[f64; 3]> = if htg.dimension() < 3 {
            vec![
        ...
        let has_zero = dists.iter().any(|&d| d.abs() <= 1e-15);
        if !has_pos || !has_neg {
            if !(htg.dimension() < 3 && has_zero) {
                continue;
            }
        }

        // Find intersection points on edges
        let edges: Vec<(usize, usize)> = if htg.dimension() < 3 {
            vec![(0, 1), (1, 2), (2, 3), (3, 0)]
        } else {
            vec![
                (0, 1),
        ...
        for &(a, b) in &edges {
            if dists[a].abs() <= 1e-15 {
                intersection_pts.push(corners[a]);
            }
        ...
        dedup_points(&mut intersection_pts);

        if intersection_pts.len() >= 3 {
            // Sort points by angle around centroid for proper polygon winding
            let cx: f64 =
                intersection_pts.iter().map(|p| p[0]).sum::<f64>() / intersection_pts.len() as f64;
            let cy: f64 =
                intersection_pts.iter().map(|p| p[1]).sum::<f64>() / intersection_pts.len() as f64;
            let cz: f64 =
                intersection_pts.iter().map(|p| p[2]).sum::<f64>() / intersection_pts.len() as f64;
            ...
            cell_idx_data.push(cell_index as f64);
        }
    }
...
fn dedup_points(points: &mut Vec<[f64; 3]>) {
    let mut unique = Vec::with_capacity(points.len());
    for point in points.drain(..) {
        let exists = unique.iter().any(|other: &[f64; 3]| {
            (point[0] - other[0]).abs() <= 1e-12
                && (point[1] - other[1]).abs() <= 1e-12
                && (point[2] - other[2]).abs() <= 1e-12
        });
        if !exists {
            unique.push(point);
        }
...
fn slice_cell_bounds(htg: &HyperTreeGrid) -> Vec<BoundingBox> {
    let leaves = htg.leaves();
    if !leaves.is_empty() {
        return leaves.into_iter().map(|leaf| leaf.bounds).collect();
    }
    ...
    let mut out = Vec::with_capacity(gs[0] * gs[1] * gs[2]);
    for k in 0..gs[2] {
        for j in 0..gs[1] {
            for i in 0..gs[0] {
                let p0 = [
                ...
                let p1 = [p0[0] + spacing[0], p0[1] + spacing[1], p0[2] + spacing[2]];
