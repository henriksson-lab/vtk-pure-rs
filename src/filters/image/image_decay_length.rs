...
pub fn image_decay_length(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n)
        .map(|i| {
            ...
            buf[0] / 3e8 * buf[0].abs().max(0.01) * 3e8
        })
        .collect();
...
mod tests {
    ...
    #[test]
    fn computes_particle_decay_length() {
        let img = image(&[-2.0, 0.0, 0.005, 3.0]);
        let r = image_decay_length(&img, "v");
