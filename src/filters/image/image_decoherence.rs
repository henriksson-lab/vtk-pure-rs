//! Quantum decoherence decay
use crate::data::{AnyDataArray, DataArray, ImageData};
pub fn image_decoherence(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n)
        .map(|i| {
            ...
            (-buf[0] * buf[0] * 0.5).exp() * ((buf[0] * 10.0).cos())
        })
        .collect();
...
mod tests {
    ...
    #[test]
    fn computes_quantum_decoherence_decay() {
        let img = image(&[-0.5, 0.0, 0.5]);
        let r = image_decoherence(&img, "v");
