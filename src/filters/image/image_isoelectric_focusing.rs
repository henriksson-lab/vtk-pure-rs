...
pub fn image_isoelectric_focusing(input: &ImageData, scalars: &str) -> ImageData {
...
mod tests {
    ...
    fn test() {
        ...
        let r = image_isoelectric_focusing(&img, "v");
        assert_eq!(r.dimensions(), [5, 5, 1]);
    }

    #[test]
    fn maps_normalized_input_to_ph_range() {
        let img = ImageData::with_dimensions(4, 1, 1).with_point_array(AnyDataArray::F64(
            DataArray::from_vec("v", vec![-1.0, 0.0, 1.0, 2.0], 1),
        ));
        let r = image_isoelectric_focusing(&img, "v");
        let arr = r.point_data().get_array("v").unwrap();
        let mut buf = [0.0f64];
        for (i, expected) in [3.0, 3.0, 10.0, 10.0].iter().enumerate() {
            arr.tuple_as_f64(i, &mut buf);
