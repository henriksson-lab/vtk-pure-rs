...
pub fn image_ising_magnetization(input: &ImageData, scalars: &str) -> ImageData {
...
mod tests {
    ...
    fn test() {
        ...
        let r = image_ising_magnetization(&img, "v");
        assert_eq!(r.dimensions(), [5, 5, 1]);
    }

    #[test]
    fn applies_hyperbolic_tangent() {
        let img = ImageData::with_dimensions(2, 1, 1).with_point_array(AnyDataArray::F64(
            DataArray::from_vec("v", vec![0.0, 1.0], 1),
        ));
        let r = image_ising_magnetization(&img, "v");
        ...
        assert!((buf[0] - 1.0f64.tanh()).abs() < 1e-12);
    }
