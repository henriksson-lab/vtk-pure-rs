...
pub fn image_deep_percolation(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n)
        .map(|i| {
            ...
            (buf[0] - 0.8 * buf[0].abs().max(0.01)).max(0.0)
        })
        .collect();
...
mod tests {
    ...
    #[test]
    fn computes_deep_percolation() {
        let img = image(&[-1.0, 0.005, 2.0]);
        let r = image_deep_percolation(&img, "v");
