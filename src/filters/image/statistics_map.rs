...
pub fn image_z_score(input: &ImageData, scalars: &str, radius: usize) -> ImageData {
    ...
    for k in 0..nz {
        for j in 0..ny {
            for i in 0..nx {
                ...
                result[k * ny * nx + j * nx + i] = if std > 1e-15 {
                    (values[k * ny * nx + j * nx + i] - mean) / std
                } else {
                    0.0
                };
            }
    ...
    let mut img = input.clone();
    img.point_data_mut()
        .add_array(AnyDataArray::F64(DataArray::from_vec("ZScore", result, 1)));
    img
}
...
mod tests {
    ...
    fn z_score_outlier() {
        ...
        values[12] = 200.0; // outlier at center
        img.point_data_mut()
