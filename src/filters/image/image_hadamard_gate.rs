...
pub fn image_hadamard_gate(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n)
        .map(|i| {
            ...
            (1.0 - 2.0 * buf[0]) / std::f64::consts::SQRT_2
        })
        .collect();
...
mod tests {
    ...
    fn test() {
        ...
        let r = image_hadamard_gate(&img, "v");
        assert_eq!(r.dimensions(), [5, 5, 1]);
    }

    #[test]
    fn computes_hadamard_basis_amplitude() {
        let img = image(&[0.0, 1.0]);
        let r = image_hadamard_gate(&img, "v");
