//! Deep water temperature profile
use crate::data::{AnyDataArray, DataArray, ImageData};
pub fn image_deep_water_temperature(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n)
        .map(|i| {
            ...
            278.0 + (buf[0] - 278.0) * (-buf[0] * 0.01).exp()
        })
        .collect();
...
mod tests {
    ...
    #[test]
    fn computes_deep_water_temperature_profile() {
        let img = image(&[278.0, 300.0]);
        let r = image_deep_water_temperature(&img, "v");
        ...
        assert_close(
            ...
            &[278.0, 278.0 + (300.0 - 278.0) * (-3.0f64).exp()],
        );
    }
}
