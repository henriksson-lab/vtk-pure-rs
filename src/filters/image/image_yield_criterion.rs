...
pub fn image_yield_criterion(input: &ImageData, scalars: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n)
        .map(|i| {
            ...
            let principal_stresses = match components {
                3 => symmetric_eigenvalues(buf[0], buf[1], 0.0, buf[2], 0.0, 0.0),
            ...
            ((s1 - s2).powi(2) + (s2 - s3).powi(2) + (s1 - s3).powi(2)).sqrt() / 2.0f64.sqrt()
        })
        .collect();
...
mod tests {
    ...
    fn test() {
        ...
        let r = image_yield_criterion(&img, "v");
    ...
    #[test]
    fn computes_2d_symmetric_tensor_von_mises() {
        let img = image(vec![3.0, 1.0, 0.0], 3);
        let r = image_yield_criterion(&img, "v");
        assert!((output_values(&r)[0] - (7.0f64).sqrt()).abs() <= 1e-12);
    }

    #[test]
    fn computes_3d_symmetric_tensor_von_mises() {
        let img = image(vec![3.0, 2.0, -1.0, 0.0, 0.0, 0.0], 6);
        let r = image_yield_criterion(&img, "v");
        assert!((output_values(&r)[0] - (13.0f64).sqrt()).abs() <= 1e-12);
    }
}
