...
pub fn joint_histogram(
    ...
    let range_a = if (max_a - min_a).abs() < 1e-15 {
        1.0
    } else {
        max_a - min_a
    };
    let range_b = if (max_b - min_b).abs() < 1e-15 {
        1.0
    } else {
        max_b - min_b
    };
...
mod tests {
    ...
    fn zero_bins_are_clamped() {
        let img = ImageData::with_dimensions(1, 1, 1)
            .with_point_array(AnyDataArray::F64(DataArray::from_vec("a", vec![1.0], 1)))
            .with_point_array(AnyDataArray::F64(DataArray::from_vec("b", vec![2.0], 1)));
        let h = joint_histogram(&img, "a", "b", 0, 0);
        assert_eq!(h.dimensions(), [1, 1, 1]);
        assert_eq!(mutual_information(&img, "missing", "b", 0), 0.0);
    }
}
