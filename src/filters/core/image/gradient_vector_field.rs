...
pub fn gradient_vector_field(input: &ImageData, scalars: &str) -> ImageData {
    ...
    for idx in 0..n {
        ...
        let gx = (vals[x_max] - vals[x_min]) * 0.5 / sp[0];
        let gy = (vals[y_max] - vals[y_min]) * 0.5 / sp[1];
        data.push(gx);
        data.push(gy);
    }
...
pub fn divergence_2d(input: &ImageData, array_name: &str) -> ImageData {
    ...
    let data: Vec<f64> = (0..n)
        .map(|idx| {
            ...
            let y_min = if iy == 0 { idx } else { idx - nx };
            let y_max = if iy + 1 == ny { idx } else { idx + nx };
            let dfdx = (vecs[x_max][0] - vecs[x_min][0]) * 0.5 / sp[0];
            let dgdy = (vecs[y_max][1] - vecs[y_min][1]) * 0.5 / sp[1];
            dfdx + dgdy
        })
        .collect();
