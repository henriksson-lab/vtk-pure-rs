...
pub fn smooth_volume_preserving(mesh: &PolyData, iterations: usize, lambda: f64) -> PolyData {
    ...
    if orig_vol < 1e-30 || mesh.points.len() == 0 {
        return mesh.clone();
    }
...
mod tests {
    use super::*;
    use crate::filters::core::sources::cube::{cube, CubeParams};
    use crate::filters::core::triangulate::triangulate;
    #[test]
    fn test_volume_preserved() {
        ...
        assert!(
            (orig_vol - new_vol).abs() / orig_vol < 0.05,
            "volume changed too much: {orig_vol} vs {new_vol}"
        );
    }
}
