    ...
    for seed_idx in 0..n_seeds {
        ...
        for (step_idx, step) in time_steps.iter().enumerate() {
            ...
            if step_idx > 0 {
                ...
                speed_data.push(if dt.abs() > 1e-30 {
                    dist / dt.abs()
                } else {
                    0.0
                });
...
fn nearest_point(points: &crate::data::Points<f64>, target: [f64; 3]) -> [f64; 3] {
...
mod tests {
    ...
    #[test]
    fn speed_is_nonnegative_for_decreasing_times() {
        let step0 = PolyData::from_triangles(vec![[0.0, 0.0, 0.0]], vec![]);
        let step1 = PolyData::from_triangles(vec![[1.0, 0.0, 0.0]], vec![]);
        let result = temporal_pathlines(&[step0, step1], &[1.0, 0.0]);
        let speed = result.point_data().get_array("Speed").unwrap();
        let mut buf = [0.0f64];
        speed.tuple_as_f64(1, &mut buf);
        assert_eq!(buf[0], 1.0);
    }
}
